...
def process_chunk(private_keys_chunk, pub_keys_set, need_test, output_file, mode):
    """Processes a chunk of private keys in parallel with different transformations."""
    results = []
    ...
    for priv in private_keys_chunk:
        for elene in need_test:
            ...
            if pub_uncompressed in pub_keys_set:
                results.append((mode, hex(test_priv)))
                print(f"✅ Match Found! Mode: {mode}, Private Key: {hex(test_priv)}")
...
def main():
    ...
    # Different modes for comparison
    modes = ["+", "*"]
    ...
    for mode in modes:
        for chunk in private_key_chunks:
            p = multiprocessing.Process(target=process_chunk, args=(chunk, pub_keys_set, need_test, output_file, mode))
            processes.append(p)
            p.start()
